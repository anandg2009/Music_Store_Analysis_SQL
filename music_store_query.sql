...
-- from customer
-- join invoice on customer.customer_id = invoice.customer
...
select artist.artist_id, artist.name, COUNT(artist.artist_id) AS number_0f_songs
from track
...
With best_selling_artist as (
    select artist.artist_id as artist_id, artist_name as artist_name,
    sum(invoice_line.unit_price*invoice_line.quantity) as total_sales
    from invoice_line
    join track on track.track_id = invoice_line_id
	join album on album.track_id = track.album_id
	join artist on artist.artist_id = album.artist_id
    group by 1
    order by 3 desc
    limit 1
...
join invoice_line il on il.track_id = i.invoice_id
...
(
    ...
    from invoice_line
    join invoice on invoice.invoice.invoice_id = invoice_line.invoice_id
